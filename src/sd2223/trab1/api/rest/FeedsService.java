...
public interface FeedsService {
	 ...
	 * user. A message should be identified before publish it, by assigning an ID. A
	 * user must contact the server of her domain directly (i.e., this operation
	 * should not be propagated to other domain)
	 *
	 ...
	 * @return 200 the unique numerical identifier for the posted message; 404 if
	 *         the publisher does not exist in the current domain 403 if the pwd is
	 *         not correct 400 otherwise
	 */
	@POST
	@Path("/{" + USER + "}")
	...
	@POST
	@Path(PROPAGATE_POST + "/{" + USER + "}")
	@Consumes(MediaType.APPLICATION_JSON)
	void postSubMessage(@PathParam(USER) String user, Message msg);

	/**
	 * Removes the message identified by mid from the feed of user. A user must
	 * contact the server of her domain directly (i.e., this operation should not be
	 ...
	 * @return 204 if ok 403 if the pwd is not correct 404 is generated if the
	 *         message does not exist in the server or if the user does not exist
	 */
	@DELETE
	@Path("/{" + USER + "}/{" + MID + "}")
	...
	@DELETE
	@Path(PROPAGATE_DELETE + "/{" + USER + "}/{" + MID + "}")
	 ...
	 *
	 * @return 200 the message if it exists; 404 if the user or the message does not
	 *         exists
	 */
	@GET
	@Path("/{" + USER + "}/{" + MID + "}")
	 ...
	 * than time (note: may be a remote user)
	 * 
	 * @param user user feed being accessed (format user@domain)
	 * @param time the oldest time of the messages to be returned
	 * @return 200 a list of messages, potentially empty; 404 if the user does not
	 *         exist.
	 */
	@GET
	@Path("/{" + USER + "}")
	...
	/**
	 * Subscribe a user. A user must contact the server of her domain directly
	 * (i.e., this operation should not be propagated to other domain)
	 *
	 ...
	 * @param pwd     password of the user
	 * @return 204 if ok 404 is generated if the user or the user to be subscribed
	 *         does not exist 403 is generated if the pwd is not correct
	 */
	@POST
	...
	@POST
	@Path(ADD_SUBSCRIBER + "/{" + USER + "}/{" + USERSUB + "}")
	@Produces(MediaType.APPLICATION_JSON)
	void addSubscriber(@PathParam(USER) String user, @PathParam(USERSUB) String userSub);

	/**
	 * UnSubscribe a user A user must contact the server of her domain directly
	 * (i.e., this operation should not be propagated to other domain)
	 *
	 ...
	 * @param pwd     password of the user
	 * @return 204 if ok 404 is generated if the user or the user to be unsubscribed
	 *         does not exist 403 is generated if the pwd is not correct
	 */
	@DELETE
	@Path("/sub/{" + USER + "}/{" + USERSUB + "}")
	...
	@DELETE
	@Path(DEL_SUBSCRIBER + "/{" + USER + "}/{" + USERSUB + "}")
	@Produces(MediaType.APPLICATION_JSON)
	 ...
	 * @return 200 if ok 404 is generated if the user does not exist
	 */
	@GET
	@Path("/sub/list/{" + USER + "}")
	@Produces(MediaType.APPLICATION_JSON)
